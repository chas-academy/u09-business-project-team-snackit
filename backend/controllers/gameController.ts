...
export const createGame = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Game created. Ready to start.", gameId: newGame._id });
  } catch (err: unknown) {
...
export const checkSubmission = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (game.currentTurn.toString() !== playerId.toString()) {
      // Kollar om det är spelaranes tur
    ...
    if (
      !recipeSearchData ||
      !recipeSearchData.results ||
      recipeSearchData.results.length === 0
    ) {
      ...
      if (game.lives[playerId.toString()] <= 0) {
        // ifall spelaren har 0 liv kvar avsluta spelet
        ...
        return res
          ...
          .json({
            result: "finished",
            message: "Game over! You lost! (Recipe not found)",
            winner: opponentId,
            score: game.score,
          });
      }
      ...
      return res
        ...
        .json({
          result: "lost",
          message: "No recipe found! You lost but try again!",
          lives: game.lives,
          score: game.score,
        });
    }
    ...
    const ingredients = recipeInfoData.extendedIngredients.map((ing: any) =>
      ing.name.toLowerCase().trim()
    ); // hämtar alla ingredienser i receptet och gör dem till små bokstäver och tar bort mellanslag
    ...
    if (!ingredientIsCorrect) {
      ...
      return res
        ...
        .json({
          result: "lost",
          message: `The recipe doesn't include ${requiredIngredient}!Try again`,
          score: game.score,
          lives: game.lives,
        });
    }
    ...
    return res
      ...
      .json({
        result: "correct",
        message: `Correct! The ingredient was included.`,
        lives: game.lives,
        score: game.score,
      });
  } catch (err: any) {
...
export const nextIngredient = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const nextTurn = game.players.find(
      (id) => id.toString() !== game.currentTurn.toString()
    ); // Hämtar motståndarens id som ska få nästa tur
    if (!nextTurn) {
    ...
    return res
      .status(200)
      .json({
        message: "Here is the next ingredient",
        nextTurn,
        nextIngredient: spoonData.usedIngredient,
        lives: game.lives,
      });
  } catch (err: unknown) {
    if (err instanceof Error) {
...
export const forfeitGame = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!game.players.map((id) => id.toString()).includes(playerId)) {
      //kontrollerar att spelaren är med i spelet
      return res
        .status(403)
    ...
    return res
      .status(200)
      .json({
        message: "You forfeited the game, you lost and you opponent won!",
        winner: opponentId,
      });
  } catch (err: unknown) {
    if (err instanceof Error) {
...
export const restartGame = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!oldGame) {
      // Kontrollerar att spelet är avslutat och för att kunna starta om spelet med samma spelare igen
      return res.status(404).json({ error: "This game not found" });
    }
    ...
    return res
      ...
      .json({
        ...
        gameId: newGame._id,
        nextIngredient: newGame.currentIngredient,
      });
  } catch (err: unknown) {
    if (err instanceof Error) {
      res.status(500).json({ error: err.message });
